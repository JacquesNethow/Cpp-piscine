...
float BitcoinExchange::getDataBaseValue(const std::map<std::string, float>& dataBase, const std::string& iDate) {
...
void BitcoinExchange::displayLine(std::string inputLine, int count) {
	...
	dbNbr = getDataBaseValue(this->dataBase, iDate);
...
std::ostream &operator<<(std::ostream &o, BitcoinExchange const &b) {
	std::map<std::string, float> db = b.getDataBase();
