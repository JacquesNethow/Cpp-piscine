...
ClapTrap::ClapTrap(void) : name("Clapper"), health(10), energy(10), attackPower(0)
{
	std::cout << this->name << " reporting for duty!" << std::endl;
}

ClapTrap::ClapTrap(std::string name) : name(name), health(10), energy(10), attackPower(0)
{
	std::cout << this->name << " reporting for duty!" << std::endl;
}
...
void ClapTrap::beRepaired(unsigned int healingAmount )
{
	...
	if (this->health >= 10)
	{
		std::cout << this->getName() << " can't heal because it's already fully healed!" << std::endl;
		return ;
	}
	if (this->health <= 0)
	{
	...
	if (this->health + healingAmount > 10)
		this->health = 10;
	else
